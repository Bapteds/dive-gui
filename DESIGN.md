...
- **NativeSelect:** the single styled native `<select>` (feature screens that need plain native selects use it instead of hand-styling). Same shape as Input: 40px, `--radius-sm`, hairline border, blue focus ring, lucide chevron, explicit surface + text colour. Never hand-roll a select.
- **ErrorState:** the shared inline "could not load" block (danger tint, `role="alert"`, title + guidance + secondary "Try again"). Every list/panel load error renders through it - never re-implement the block.
- **Empty state variants:** `EmptyState` renders its own bordered card by default; `variant="inline"` drops the card chrome for use INSIDE an existing panel so empty panels never nest a card in a card.
- **Dropdown menu / Tooltip / Avatar / Separator:** Radix primitives, tokenised. Tooltips explain disabled actions and icon-only buttons. Menu/select items use `--radius-xs` inside their `--radius-sm` container (concentric radii).
- **Terminal (dark inset):** the app's ONE dark surface. Its palette lives in the `--terminal-*` tokens (ANSI slots stay on the brand scale; no violet/cyan decorative hues).
